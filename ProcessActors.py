...
def get_gold_skulls(rom) -> dict[int, tuple[int, int, int, int, str, dict[str, str]]]:
    def get_gold_skull_func(rom, actor_id, actor, scene, room_id, setup_num, actor_num):
        if actor_id == 0x0095:
            gold_skull = (scene, room_id, setup_num, actor_num, scenes[scene], process_gold_skull(rom.read_bytes(actor, 16)))
            if gold_skull[5]["item_id"] == "Gold Skulltula" or gold_skull[5]["item_id"] == "Night Gold Skulltula":
                return gold_skull
    return get_actor_list(rom, get_gold_skull_func)
...
def process_gold_skull(actor_bytes):
    variable = (actor_bytes[14] << 8) + actor_bytes[15]
    item_id = variable & 0xE000
    item_dict = {
        0x0000: "Skullwalltula",
        0x8000: "Gold Skulltula",
        0xA000: "Night Gold Skulltula",
    }
    return {
        "variable": hex(variable),
        "item_id": item_dict[item_id]
...
for (index, (key, value)) in enumerate(gold_skulls.items()):
